...
let isConnected = false, isCapturing = false, currentSessionId = null;
function updateGuiState() {
    if (!isConnected) {
        statusDot.className = 'status-dot'; connectionStatusSpan.textContent = 'Disconnected'; connectionStatusSpan.className = 'text-sm font-medium text-red-400';
        startButton.disabled = true; startButton.classList.add('opacity-50'); stopButton.disabled = true; stopButton.classList.add('opacity-50'); endSessionButton.disabled = true; endSessionButton.classList.add('opacity-50');
    } else if (isCapturing) {
        statusDot.className = 'status-dot capturing'; connectionStatusSpan.textContent = `Capturing (Session ${currentSessionId})...`; connectionStatusSpan.className = 'text-sm font-medium text-yellow-400';
        startButton.disabled = true; startButton.classList.add('opacity-50'); stopButton.disabled = false; stopButton.classList.remove('opacity-50'); endSessionButton.disabled = false; endSessionButton.classList.remove('opacity-50');
    } else {
        statusDot.className = 'status-dot connected'; connectionStatusSpan.textContent = 'Connected (Idle)'; connectionStatusSpan.className = 'text-sm font-medium text-green-400';
        startButton.disabled = false; startButton.classList.remove('opacity-50'); stopButton.disabled = true; stopButton.classList.add('opacity-50');
        endSessionButton.disabled = currentSessionId === null;
        if (currentSessionId === null) { endSessionButton.classList.add('opacity-50'); } else { connectionStatusSpan.textContent = `Paused (Session ${currentSessionId})`; endSessionButton.classList.remove('opacity-50'); }
    }
}
// --- END: STATE MANAGEMENT ---
let websocket = null, protocolChart = null, endpointChart = null, timelineChart = null, wifiChart = null, dataVolumeChart = null, serviceChart = null, activityTrendChart = null, globalProtocolChart = null, globalTopTalkersChart = null;
let totalPacketCount = 0, totalDataVolume = 0, securityAlertCount = 0;
let bytesThisSecond = 0;
const WS_URL = 'ws://localhost:8765', MAX_PACKETS_DISPLAY = 1000, packetCache = [];
const packetTableBody = document.getElementById('packet-table-body'), packetCountSpan = document.getElementById('packet-count'), navButtons = document.querySelectorAll('.nav-button');
const packetDetailModal = document.getElementById('packet-detail-modal'), modalTitle = document.getElementById('modal-title'), modalBody = document.getElementById('modal-body');
const historyTableBody = document.getElementById('history-table-body');
const protocolCounts = {}, endpointStats = {}, wifiFrameCounts = {}, portCounts = {};
const uniqueDevices = new Set();
const CHART_COLORS = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#6366F1', '#14B8A6'];
let network = null; const nodes = new vis.DataSet([]); const edges = new vis.DataSet([]);
let tableUpdateBuffer = []; let chartUpdateNeeded = false;
const discoveredEnvNetworks = {}; const discoveredEnvDevices = {};
...
function connectWebSocket() {
    ...
    websocket.onmessage = (event) => {
        try {
            ...
            } else handlePacket(data);
        } catch (error) { console.error("Failed to parse message:", error); }
    };
}
function startCapture() {
    if (!websocket || websocket.readyState !== WebSocket.OPEN) return;
    if (!isCapturing && currentSessionId === null) { clearAllData(); }
...
function stopCapture() { if (websocket && websocket.readyState === WebSocket.OPEN) { websocket.send(JSON.stringify({ action: 'STOP' })); } isCapturing = false; updateGuiState(); }
function endSession() { if (websocket && websocket.readyState === WebSocket.OPEN && currentSessionId !== null) { if (confirm(`End session ${currentSessionId}?`)) { websocket.send(JSON.stringify({ action: 'END_SESSION' })); } } }
function clearAllData() {
    ...
    updateSummaryWidgets(); clearAggregationData();
    ...
    document.getElementById('security-events-body').innerHTML = ''; document.getElementById('alerts-log-body').innerHTML = '';
    nodes.clear(); edges.clear(); initCharts();
}
function handlePacket(data) {
    totalPacketCount++; packetCountSpan.textContent = totalPacketCount; totalDataVolume += data.summary.length; bytesThisSecond += data.summary.length;
    packetCache.unshift(data); if (packetCache.length > MAX_PACKETS_DISPLAY) { packetCache.pop(); }
    addPacketToTable(data.summary, totalPacketCount);
    const summary = data.summary;
    uniqueDevices.add(summary.source); uniqueDevices.add(summary.destination);
    updateSummaryWidgets();
    updateProtocolStats(summary.protocol); updateEndpointStats(summary.source, summary.length); updateEndpointStats(summary.destination, summary.length);
...
function addPacketToTable(summary, packetNumber) {
    ...
    row.dataset.cacheIndex = 0; row.onclick = () => showPacketDetails(0);
...
function showPacketDetails(cacheIndex) {
    const packetData = packetCache[cacheIndex]; if (!packetData || !packetData.full_packet) { return; }
    const packetNumber = totalPacketCount - cacheIndex; modalTitle.textContent = `Details for Packet #${packetNumber}`;
    modalBody.innerHTML = ''; const { full_packet } = packetData;
    for (const layerName in full_packet) {
        if (layerName === 'raw_payload') continue; const fields = full_packet[layerName];
        const details = document.createElement('details'); details.open = true;
        const summaryElem = document.createElement('summary'); summaryElem.textContent = layerName.split(':')[1].replace(/_/g, ' ').toUpperCase();
        details.appendChild(summaryElem);
        for (const [key, value] of Object.entries(fields)) {
            const fieldDiv = document.createElement('div'); fieldDiv.className = 'field'; fieldDiv.innerHTML = `<span class="field-key">${key}</span><span class="field-value">${value}</span>`;
            details.appendChild(fieldDiv);
        }
        modalBody.appendChild(details);
    }
    if (full_packet.raw_payload) {
        const details = document.createElement('details'); details.open = true; const summaryElem = document.createElement('summary'); summaryElem.textContent = 'RAW PAYLOAD';
        details.appendChild(summaryElem); const pre = document.createElement('pre'); pre.className = 'payload-hexdump'; pre.textContent = full_packet.raw_payload;
        details.appendChild(pre); modalBody.appendChild(details);
    }
    packetDetailModal.classList.remove('hidden');
}
function closePacketDetails() { packetDetailModal.classList.add('hidden'); }
function applyDisplayFilter() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i]; const packet = packetCache[i]?.summary; if (!packet) continue;
        const text = `${packet.protocol} ${packet.source} ${packet.destination} ${packet.info}`.toLowerCase();
        row.style.display = text.includes(filterText) ? '' : 'none';
    }
}
function showPage(pageId) {
    document.querySelectorAll('.page-content').forEach(p => p.classList.add('hidden')); document.getElementById(`${pageId}-page`).classList.remove('hidden');
    navButtons.forEach(b => {
        b.classList.toggle('text-indigo-400', b.dataset.page === pageId); b.classList.toggle('bg-[#21262d]', b.dataset.page === pageId); b.classList.toggle('text-gray-400', b.dataset.page !== pageId);
    });
    if (pageId === 'global-stats') { websocket.send(JSON.stringify({ action: 'GET_GLOBAL_ANALYSIS' })); }
}
navButtons.forEach(b => b.addEventListener('click', () => showPage(b.dataset.page)));
function updateInterfaceList(interfaces) {
    const select = document.getElementById('interface-select'); select.innerHTML = '';
    interfaces.forEach(iface => {
        const option = document.createElement('option'); option.value = iface; option.textContent = iface; if (iface.includes('mon')) option.selected = true; select.appendChild(option);
    });
}
function updateHistoryList(sessions) {
    historyTableBody.innerHTML = ''; if (sessions.length === 0) { historyTableBody.innerHTML = '<tr><td colspan="5" class="text-center text-gray-500 py-4">No saved sessions found.</td></tr>'; return; }
    sessions.forEach(session => {
        const row = document.createElement('tr'); const statusColor = session.status === 'Finished' ? 'text-green-400' : 'text-yellow-400';
...
function loadSession(sessionId) { if (!websocket || websocket.readyState !== WebSocket.OPEN) return; clearAllData(); showPage('dashboard'); packetTableBody.innerHTML = '<tr><td colspan="7" class="text-center text-gray-500 py-4">Loading packets...</td></tr>'; websocket.send(JSON.stringify({ action: 'GET_SESSION_PACKETS', session_id: sessionId })); }
function downloadSession(sessionId) { if (!websocket || websocket.readyState !== WebSocket.OPEN) return; websocket.send(JSON.stringify({ action: 'DOWNLOAD_SESSION', session_id: sessionId })); }
function triggerDownload(data) { const jsonData = JSON.stringify(data.data, null, 2); const blob = new Blob([jsonData], { type: 'application/json' }); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.style.display = 'none'; a.href = url; a.download = `session_${data.session_id}.json`; document.body.appendChild(a); a.click(); window.URL.revokeObjectURL(url); document.body.removeChild(a); }
function loadSessionPackets(packets) {
    clearAllData(); showPage('dashboard');
    setTimeout(() => { packets.forEach(packet => { if (packet) handlePacket(packet); }); showPage('live-capture'); }, 100);
}
function deleteSession(sessionId) { if (!websocket || websocket.readyState !== WebSocket.OPEN) return; if (confirm(`Are you sure you want to delete session ${sessionId}?`)) { websocket.send(JSON.stringify({ action: 'DELETE_SESSION', session_id: sessionId })); } }
function initCharts() {
    ...
    const onChartClick = (event, elements) => { if (elements.length > 0) { const chart = elements[0].element.$context.chart; const label = chart.data.labels[elements[0].index]; const ipAddress = label.split(' ')[0]; document.getElementById('display-filter-input').value = ipAddress; applyDisplayFilter(); showPage('live-capture'); } };
    ...
    serviceChart = new Chart(document.getElementById('service-chart').getContext('2d'), { type: 'bar', data: { labels: [], datasets: [{ label: 'Packets', data: [], backgroundColor: CHART_COLORS[4], borderWidth: 0 }] }, options: { ...barOptionsWithClick, onClick: null } });
...
function updateSystemStats({ cpu, memory }) { if(document.getElementById('cpu-gauge-text')) { document.getElementById('cpu-gauge-text').textContent = `${cpu.toFixed(1)}%`; document.getElementById('cpu-gauge-bar').style.strokeDasharray = `${cpu}, 100`; document.getElementById('mem-gauge-text').textContent = `${memory.toFixed(1)}%`; document.getElementById('mem-gauge-bar').style.strokeDasharray = `${memory}, 100`; } }
function updateTimelineChart() { if (!timelineChart || !isCapturing) return; const now = Date.now(); const data = timelineChart.data.datasets[0].data; const megabitsPerSecond = (bytesThisSecond * 8) / 1000000; data.push({ x: now, y: megabitsPerSecond.toFixed(2) }); bytesThisSecond = 0; if (data.length > 60) data.shift(); timelineChart.update('quiet'); }
...
function handleSecurityEvent(event) { securityAlertCount++; updateSummaryWidgets(); const tableBody = document.getElementById('security-events-body'); const row = document.createElement('tr'); row.innerHTML = `<td class="py-2">${new Date(event.time * 1000).toLocaleTimeString()}</td><td class="py-2 text-red-400">${event.type}</td><td class="py-2">${event.message}</td>`; tableBody.prepend(row); }
...
function handleAlertTriggered(data) { const tableBody = document.getElementById('alerts-log-body'); const row = document.createElement('tr'); row.innerHTML = `<td class="py-2">${new Date().toLocaleTimeString()}</td><td class="py-2 text-yellow-400">${data.type}</td><td class="py-2">${data.message}</td>`; tableBody.prepend(row); }
function saveAlertSettings() {
    const thresholds = { cpu: parseInt(document.getElementById('alert-cpu-threshold').value), throughput: parseInt(document.getElementById('alert-throughput-threshold').value), deauthAttack: document.getElementById('alert-deauth-checkbox').checked, arpSpoofing: document.getElementById('alert-arp-checkbox').checked };
    localStorage.setItem('alertThresholds', JSON.stringify(thresholds));
    if (websocket && websocket.readyState === WebSocket.OPEN) { websocket.send(JSON.stringify({ action: 'UPDATE_ALERT_THRESHOLDS', thresholds: thresholds })); }
...
function loadAlertSettings() {
    ...
    if (saved) {
        ...
        document.getElementById('alert-cpu-threshold').value = thresholds.cpu; document.getElementById('alert-throughput-threshold').value = thresholds.throughput;
        document.getElementById('alert-deauth-checkbox').checked = thresholds.deauthAttack; document.getElementById('alert-arp-checkbox').checked = thresholds.arpSpoofing;
        if (websocket && websocket.readyState === WebSocket.OPEN) { websocket.send(JSON.stringify({ action: 'UPDATE_ALERT_THRESHOLDS', thresholds: thresholds })); }
    }
...
function renderChartUpdates() {
    ...
    protocolChart.data.labels = sortedProtocols.map(e => e[0]); protocolChart.data.datasets[0].data = sortedProtocols.map(e => e[1]); protocolChart.update();
    const sortedEndpoints = Object.entries(endpointStats).sort(([, a], [, b]) => b.packets - a.packets).slice(0, 10);
    endpointChart.data.labels = sortedEndpoints.map(e => e[0]); endpointChart.data.datasets[0].data = sortedEndpoints.map(e => e[1].packets); endpointChart.update();
    const sortedVolume = Object.entries(endpointStats).sort(([, a], [, b]) => b.bytes - a.bytes).slice(0, 10);
    dataVolumeChart.data.labels = sortedVolume.map(e => e[0]); dataVolumeChart.data.datasets[0].data = sortedVolume.map(e => (e[1].bytes / 1048576).toFixed(2)); dataVolumeChart.update();
    const sortedPorts = Object.entries(portCounts).sort(([, a], [, b]) => b - a).slice(0, 10);
    serviceChart.data.labels = sortedPorts.map(e => portMap[e[0]] ? `${portMap[e[0]]} (${e[0]})` : e[0]); serviceChart.data.datasets[0].data = sortedPorts.map(e => e[1]); serviceChart.update();
    const sortedWifi = Object.entries(wifiFrameCounts).sort(([, a], [, b]) => b - a);
    wifiChart.data.labels = sortedWifi.map(e => e[0]); wifiChart.data.datasets[0].data = sortedWifi.map(e => e[1]); wifiChart.update();
...
document.addEventListener('DOMContentLoaded', () => {
    connectWebSocket(); showPage('dashboard'); initCharts(); initNetworkGraph(); updateGuiState();
    loadAlertSettings();
    const autoScrollCheckbox = document.getElementById('auto-scroll-checkbox');
    setInterval(() => {
        if (tableUpdateBuffer.length === 0) return;
        if (autoScrollCheckbox.checked) {
            const fragment = document.createDocumentFragment(); packetTableBody.prepend(...tableUpdateBuffer);
            while (packetTableBody.children.length > MAX_PACKETS_DISPLAY) { packetTableBody.removeChild(packetTableBody.lastChild); }
            tableUpdateBuffer = []; applyDisplayFilter();
        }
    }, 500);
    setInterval(renderChartUpdates, 1000); setInterval(updateTimelineChart, 1000);
});